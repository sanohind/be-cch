...
class Block9OutflowController extends Controller
{
    ...
    public function show($id): JsonResponse
    {
        ...
        $headers = CchOutflowPic::with(['picUser:id,full_name,username,division_id'])
            ->where('cch_id', $id)
            ->orderByDesc('updated_at')
            ->get();

        $record = CchOutflowPic::with(['picUser:id,full_name,username,division_id', 'process', 'supplier', 'supplierProcess'])
            ->where('cch_id', $id)
